...
const RegistrationAndLoginForm = () => {
  ...
  useEffect(() => {
    if (currentId === '')
      ...
      setValues({
        ...usersObjects[currentId]
      })
  }, [currentId, usersObjects])
  ...
  const handleFormSubmit = e => {
    ...
    if(!values.firstname) {
      toastr.clear()
      setTimeout(() => toastr.error(`First Name Cannot be empty`), 300)
      return;
    }
    if(!values.lastname) {
      toastr.clear()
      setTimeout(() => toastr.error(`Last Name Cannot be empty`), 300)
      return;
    }
    if(!values.email) {
      toastr.clear()
    ...
    if(!values.day) {
      toastr.clear()
      setTimeout(() => toastr.error(`Date Cannot be empty`), 300)
      return;
    }
    if(!values.month) {
      toastr.clear()
      setTimeout(() => toastr.error(`Month Cannot be empty`), 300)
      return;
    }
    if(!values.year) {
      toastr.clear()
      setTimeout(() => toastr.error(`Year Cannot be empty`), 300)
      return;
    }
    if(!values.gender) {
      toastr.clear()
      setTimeout(() => toastr.error(`Gender Cannot be empty`), 300)
      return;
    }
    if(!values.city) {
      toastr.clear()
      setTimeout(() => toastr.error(`City Cannot be empty`), 300)
      return;
    }
    if(!values.country) {
      toastr.clear()
